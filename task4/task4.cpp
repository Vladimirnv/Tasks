...
public:
    ...
    bool Aes_Crypto(std::vector<byte> data, std::vector<byte> key, CryptoPP::CipherDir direction)
    {
        _data = data;
        ...
        {
            cryptoalg = new CryptoPP::AESEncryption(key.data(), CryptoPP::AES::DEFAULT_KEYLENGTH);
        }
        else
        {
            cryptoalg = new CryptoPP::AESDecryption(key.data(), CryptoPP::AES::DEFAULT_KEYLENGTH);
        }
...
int main()
{
    ...
    while (loop)
    {
        ...
        if (cryptoflag)
        {
            ...
            {
                if (!aes.Aes_Crypto(plaintext.GetData(), keytext.GetData(), mode))
                {
                    std::cout << _ERROR_ENCRIPT << std::endl;
                }
                else
                {
                    plaintext.SetData(aes.GetData());
