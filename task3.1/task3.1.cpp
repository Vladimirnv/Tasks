...
public:
    ...
    bool _3Des_Crypto(std::vector<byte> data, std::vector<byte> key, CryptoPP::CipherDir direction)
    {
        _data = data;
        CryptoPP::BlockTransformation *cryptoalg = NULL;
        if (direction == CryptoPP::DECRYPTION)
        {
            cryptoalg = new CryptoPP::DES_EDE3_Encryption(key.data(), CryptoPP::DES_EDE3::KEYLENGTH);
        }
        else
        {
            cryptoalg = new CryptoPP::DES_EDE3_Decryption(key.data(), CryptoPP::DES_EDE3::KEYLENGTH);
        }
...
int main()
{
    ...
    while (loop)
    {
        ...
        if (cryptoflag)
        {
            ...
            {
                if (!des._3Des_Crypto(plaintext.GetData(), keytext.GetData(), mode))
                {
